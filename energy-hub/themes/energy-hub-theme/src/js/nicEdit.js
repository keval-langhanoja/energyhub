...
var nicEditorConfig = bkClass.extend({
	...
	iconsPath: '/o/energy-hub-theme/images/richtext/nicEditorIcons.gif',
	buttonList: ['save', 'bold', 'italic', 'underline', 'left', 'center', 'right',
		'justify', 'ol', 'ul',  'indent', 'outdent', 'link', 'unlink'],
	iconList: {
		"bgcolor": 1,
		"forecolor": 2,
		"bold": 3,
...
var nicEditor = bkClass.extend({
	...
	panelInstance: function (B, C) {
		...
		var A = new bkElement("DIV").setStyle({
			width: "100%"//(parseInt(B.getStyle("width")) || B.clientWidth) + "px" edited
		}).appendBefore(B);
		this.setPanel(A);
...
var nicEditorInstance = bkClass.extend({
	...
	construct: function (G, D, C) {
		...
		if (H || this.options.hasPanel) {
			var B = (bkLib.isMSIE && !((typeof document.body.style.maxHeight != "undefined") &&
				document.compatMode == "CSS1Compat"));
			var E = {
				width: "100%",//newX + "px", edited
				border: "0.5px solid #989898",//edited
				borderTop: 0,
				overflowY: "auto",
				overflowX: "hidden"
			};
		...
		if (C.startContainer) {
			...
			if (C.cloneContents().childNodes.length == 1) {
				for (var B = 0; B < D.childNodes.length; B++) {
					...
					if (C.compareBoundaryPoints(Range.START_TO_START, A) != 1 &&
