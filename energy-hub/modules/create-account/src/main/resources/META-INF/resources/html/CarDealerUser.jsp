	                   ...
	                   <div class="card info">
	                       <div class="d-flex">
	                           ...
	                           <div class="choosenUserDetails py-2 px-4">
								<span class=""><liferay-ui:message key="YouAreRegistering" /> <span class="bold"><liferay-ui:message key="CarDealerUser" /></span></span>
...
<!--                            </div> -->
                       ...
                       <div class="separator"></div>
                           <h1> <liferay-ui:message key="CarDealerInformation" /></h1>
                        <div class="row">
                           <div class="col-12 col-md-6 col-xl-4 mt-4 required inputContainer">
                           ...
                           <div class="col-12 col-md-6 col-xl-4 mt-4 required inputContainer">
							<label><liferay-ui:message key="GoogleMapLink" /></label>
						     <aui:input cssClass="" label="" id="googleMapLink" name="googleMapLink"></aui:input>
                           </div>
                           <div class="col-12 col-md-6 col-xl-4 mt-4 required inputContainer">
							...
							<label><liferay-ui:message key="OtherCarMake" /></label>
						     <aui:input cssClass="" label="" id="otherCarMake" name="otherCarMake" disabled="true"></aui:input>
                           </div>
                          </div>
                        </div>
...
<script>
	...
	$('#<portlet:namespace/>carMake').on('change', function() {
		var list = $('#<portlet:namespace/>carMake').val();
		var otherId = $('#<portlet:namespace/>carMake option:contains("Other")').val()
  	  	if(list.includes(otherId)) {
  	  		$('#<portlet:namespace/>otherCarMake').prop("disabled", false);
	  	  	$('#otherDiv').addClass("required");
  	  	}
  	  	if(!list.includes(otherId)){
  			$('#<portlet:namespace/>otherCarMake').val('').change();
  	  		$('#<portlet:namespace/>otherCarMake').prop("disabled", true);
	...
	if(${edit} == true) {
		...
		document.getElementById("<portlet:namespace />googleMapLink").value = "${userData.googleMapLink}";
...
// 		document.getElementById("<portlet:namespace />SubscribeToNewsletters").value = "${userData.SubscribeToNewsletters}";
		...
		var act = "${userData.carMake}";
		$.each(act.replaceAll('/', ',').split(","), function(i,e){
			$("#<portlet:namespace />carMake option[value='" + e + "']").prop("selected", true);
		});
		if("${userData.otherCarMake}" !=""){
			$("#<portlet:namespace />carMake").val("other").change();
		}else $("#<portlet:namespace />carMake").val("${userData.carMake}").change();
		document.getElementById("<portlet:namespace />otherCarMake").value = "${userData.otherCarMake}";
...
//  			 <portlet:namespace />SubscribeToNewsletters: $("#<portlet:namespace/>SubscribeToNewsletters").val(),
 			 ...
 			 <portlet:namespace />carMake: $("#<portlet:namespace/>carMake").val(),
 			 <portlet:namespace />otherCarMake: $("#<portlet:namespace/>otherCarMake").val(),
 			 <portlet:namespace />googleMapLink: $("#<portlet:namespace/>googleMapLink").val(),
