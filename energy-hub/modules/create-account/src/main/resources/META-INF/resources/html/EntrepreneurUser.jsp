                   ...
                   <div class="card info">
                       <div class="d-flex">
                           ...
                           <div class="choosenUserDetails py-2 px-4">
                               <c:choose>
								    <c:when test="${edit}">
								    	<span class=""><liferay-ui:message key="ContributeToOurCommunityBy"/> :</span>
								    	<button class="blueBorderBtn mt-3 mx-3" id="innovationProg"
								    		onclick="location.href = '/innovation?p_r_p_categoryId=${categId}';"><liferay-ui:message key="AddInnovationProgram" />	</button>
								    </c:when>
									<c:when test="${!edit}">
										<span class=""><liferay-ui:message key="YouAreRegistering" /> <span class="bold"><liferay-ui:message key="EntrepreneurUser" /></span></span>
                       ...
                       <div class="separator"></div>
                        ...
                        <div class="row">
                            <div class="col-12 col-md-6 mt-4 description inputContainer required">
								<label><liferay-ui:message key="SolutionOfferingDescription" /></label>
                            ...
                            <div class="col-12 col-md-6">
                           		<div class="col-12 mt-4 inputContainer required">
                                    ...
                                    <aui:select cssClass="browser-default custom-select custom-select-lg mb-4 signup_select"
			                            label="" id="Role" name="Role"> 
			                            <aui:option disabled="true" selected="true" value=""><liferay-ui:message key="SelectOption" /></aui:option>
			                            <c:forEach var="elt" items="${Role}">
										<aui:option value="${elt.Id}">
											<c:choose>
												<c:when test="${lang == 'en_US'}">
													${elt.Text}
                                ...
                                <div class="col-12 mt-4 inputContainer required">
									<label><liferay-ui:message key="MaturityLevelOfSolution" /></label>
                                    <aui:select cssClass="browser-default custom-select custom-select-lg mb-4 signup_select"
			                            label="" 
			                            id="MaturityLevelOfSolution" name="MaturityLevelOfSolution"> 
			                            <aui:option disabled="true" selected="true" value=""><liferay-ui:message key="SelectOption" /></aui:option>
			                            <c:forEach var="elt" items="${MaturityLevelOfSolution}">
											<aui:option value="${elt.Id}">
												<c:choose>
													<c:when test="${lang == 'en_US'}">
														${elt.Text}
                                ...
                                <div class="col-12 mt-4 inputContainer required">
									<label><liferay-ui:message key="ChallengeToSolve" /></label>
                                   <aui:select cssClass="browser-default custom-select custom-select-lg mb-4 signup_select"
			                            label="" 
			                            id="ChallengeToSolve" name="ChallengeToSolve">
			                            <aui:option disabled="true" selected="true" value=""><liferay-ui:message key="SelectOption" /></aui:option>
			                            <c:forEach var="elt" items="${ChallengeToSolve}">
											<aui:option value="${elt.Id}">
												<c:choose>
													<c:when test="${lang == 'en_US'}">
														${elt.Text}
                                ...
                                <div class="col-12 mt-4 inputContainer required">
									<label><liferay-ui:message key="SDGSolution" /></label>
                                   <aui:select cssClass="browser-default custom-select custom-select-lg mb-4 signup_select"
			                            label=""  multiple="true"
			                            id="SDGSolution" name="SDGSolution"> 
			                            <c:forEach var="elt" items="${SDGSolution}">
											<aui:option value="${elt.Id}">
												<c:choose>
													<c:when test="${lang == 'en_US'}">
														${elt.Text}
	                            ...
	                            <div class="col-12 mt-4 inputContainer required">
									<label><liferay-ui:message key="SelectTypeOfSupport" /></label>
                                    <aui:select cssClass="browser-default custom-select custom-select-lg mb-4 signup_select"
			                            label="" 
...
// 		document.getElementById("<portlet:namespace />SubscribeToNewsletters").value = "${userData.SubscribeToNewsletters}";
		...
		document.getElementById("<portlet:namespace />SolutionOfferingDescription").value = "${userData.SolutionOfferingDescription}";
		...
		$("#<portlet:namespace />MaturityLevelOfSolution").val("${userData.MaturityLevelOfSolution}").change(); 
		$("#<portlet:namespace />ChallengeToSolve").val("${userData.ChallengeToSolve}").change(); 
		$("#<portlet:namespace />Role").val("${userData.Role}").change(); 
		
	 	var supp = "${userData.TypeOfSupport}";
		$.each(supp.replaceAll('/', ',').split(","), function(i,e){
		    $("#<portlet:namespace />TypeOfSupport option[value='" + e + "']").prop("selected", true);
		});
		 
	 	var sdg = "${userData.SDGSolution}";
		$.each(sdg.replaceAll('/', ',').split(","), function(i,e){
		    $("#<portlet:namespace />SDGSolution option[value='" + e + "']").prop("selected", true);
		});
	}
...
//  			 <portlet:namespace />SubscribeToNewsletters: $("#<portlet:namespace/>SubscribeToNewsletters").val(),
 			 ...
 			 <portlet:namespace />MaturityLevelOfSolution: $("#<portlet:namespace/>MaturityLevelOfSolution").val(),
 			 <portlet:namespace />ChallengeToSolve: $("#<portlet:namespace/>ChallengeToSolve").val(),
 			 <portlet:namespace />SDGSolution: $("#<portlet:namespace/>SDGSolution").val(),
 			 <portlet:namespace />TypeOfSupport: $("#<portlet:namespace/>TypeOfSupport").val(),
 			 <portlet:namespace />Role: $("#<portlet:namespace/>Role").val()
 		};
