...
<div class="d-flex justify-content-between innovation-item-detail"> 
	<c:forEach var="news" items="${newsList}"> 
		<div style="background-color: white; padding:2rem;width: 100%">
		    <div class="titleItemDetail d-flex"><strong>${news.title}</strong></div>
		    <div class="row m-t-10">
		        <div class="col-4 inline-flex" style="width: 100%;">
		            <img class="author-profile" src="${news.userImageURL}" 
		            onerror="this.src='/o/energy-hub-theme/images/no-image.png'; this.onerror=null;"/>
		            <c:if test="${news.author !=''}">
			            <p class="description-center grey-font">${news.author} 
			            	<c:if test="${news.createDate !=''}">
			            		<liferay-ui:message key="On" /> ${news.createDate} 
				            	<c:if test="${news.modifiedDate !=''}">
				          		 (<liferay-ui:message key="UpdatedOn" /> ${news.modifiedDate})
				            	</c:if>
				            </c:if>
			            </p>
		             </c:if>
		    ...
		    <div class="row d-flex">
		        <img src="${news.imageURL}" onerror="this.src='/o/energy-hub-theme/images/no-image.png'; this.onerror=null;"/>
		        <div class="captionPart mt-5">${news.description}</div>
		    </div>
		</div>
		        ...
		        <c:forEach var="topic" items="${similarTopics}"> 
					<div class="serviceProviders-slider-items">
						<div class="card card-small">
							...
							onerror="this.src='/o/energy-hub-theme/images/no-image.png'; this.onerror=null;">
							<div class="card-body pb-2 mt-2 dets">
