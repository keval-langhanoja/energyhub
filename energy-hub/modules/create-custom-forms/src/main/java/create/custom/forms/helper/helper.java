package create.custom.forms.helper;
...
public class helper {
	...
	public static JSONObject fileUploadByDL(File file ,ThemeDisplay themeDisplay,
			ResourceRequest request, String folderName, String fileTitleOrFileId, boolean isCar, String FileEntryId)
	{
		String FileURL = "";  
		...
		long repositoryId = themeDisplay.getScopeGroupId();
	 	...
	 	try {
	    	...
	    	if( file == null ) {
	    		if(isCar) {
	    			dlFileEntry = DLFileEntryLocalServiceUtil.getFileEntry(themeDisplay.getLayout().getGroupId(), 
	    					dlFolder.getFolderId(), "DeafultCar");
	    			fileId = dlFileEntry.getFileEntryId();
	    		}else {
	    			fileId = Long.valueOf(fileTitleOrFileId);
	    			dlFileEntry = DLFileEntryLocalServiceUtil.fetchDLFileEntry(fileId);
	    		}
	    	} else {
	    		String fileTitle = fileTitleOrFileId;
	    		InputStream is = null;
	    		if(isCar && !isEmpty(FileEntryId)) {
